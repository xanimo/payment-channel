...
key refused rather than resolved to the first of two answers. hex is read in
either case, and `ref` is canonicalised to lowercase on ingest so a txid is the
same outpoint whichever case it arrives in. it does still ignore leading and
trailing text around the object. a general json parser here would be a
